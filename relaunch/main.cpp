...
{
	static Font font("Segoe UI", int(24 * dpiFactor()));
	...
	if (area.height() < area.width())
	{
		...
		oui::verticalSlider(right.popRight(20), intensity, input, colors::white);
	...
	for (int i = 0; i < 8; ++i)
	{
		auto row = area.popTop(Ratio(1.0f / (8 - i)));
		for (int j = 0; j < 8; ++j)
		{
			const auto button_area = row.popLeft(Ratio(1.0f / (8 - j)));
